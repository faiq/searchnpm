...
function fixQuery (searchObj, queryObj){ 
  ...
  //  var str = '_score' + ':' + searchObj.sortOrd 
  //  retObj.sort = [str]
    return retObj 
  }else{
    retObj.body = queryObj 
...
exports.searchEcosystem = function (searchObj){ 
  var queryObj = {}
  var terms = searchObj.searchQuery.split(" ")
  var ecokeywords = getEcowords(searchObj.ecosystem)
  console.log(ecokeywords) 
  queryObj.query = { 
    bool:{ 
      must: [ 
        {
          match: {
            keywords: ecokeywords
          }
        }
      ]
      ,
  ...
  console.log(util.inspect((fixQuery(searchObj, queryObj))
, false, null));

  return fixQuery(searchObj, queryObj)
}

function getEcowords(ecosystem){ 
  switch(ecosystem){
    case 'gulp':
      return 'gulpplugin,gulpfriendly'
    case 'grunt': 
      return 'gruntplugin'
    default: 
      return '*' 
